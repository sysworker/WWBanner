...
@interface WWBannerView()<UICollectionViewDelegate,UICollectionViewDataSource>{
    BOOL beganDragging;
    CGFloat marginTime;
}
@property(strong,nonatomic)UICollectionView *myCollectionV;
@property(strong,nonatomic)UICollectionViewFlowLayout *flowL ;
@property(strong,nonatomic)WWBannerControl *bannerControl ;
@property(strong,nonatomic)NSArray *data;
@property(strong,nonatomic)NSTimer *timer;
@property(strong,nonatomic)UIView *line;
@property(assign,nonatomic)NSInteger lastIndex;
@end
@implementation WWBannerView
...
- (void)setUp{
    ...
    if (self.param.wFadeOpen) {
        self.flowL = [[WWBannerFadeLayout alloc] initConfigureWithModel:self.param];
    }else if (self.param.wCardOverLap) {
        if (self.param.wScaleFactor == 0.5) {
            self.param.wScaleFactor = 0.8f;
        }
        self.flowL = [[WWBannerOverLayout alloc] initConfigureWithModel:self.param];
    }else{
        self.flowL = [[WWBannerFlowLayout alloc] initConfigureWithModel:self.param];
    }
...
- (void)fadeAction{
    if (self.param.wFadeOpen) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            WWBannerFadeLayout *fade = (WWBannerFadeLayout*)self.flowL;
            if (![fade isKindOfClass:[WWBannerFadeLayout class]]) {
                return;
            }
...
- (void)setParam:(WWBannerParam *)param{
    _param = param;
//    self.icon.contentMode = param.wImageFill?UIViewContentModeScaleAspectFill:UIViewContentModeScaleAspectFit;
    self.icon.contentMode = self.param.wContentMode;
}
@end
